...
(function () {
  ...
  function deletePost(postId) {
    ...
    if (!profile || !sb) {
      console.error("🔥 [ERROR] Profile or Supabase client missing!");
      showLoginModal();
      return;
    }

    // Optional fix for the aria-hidden warning: unfocus the button
    if (document.activeElement) { document.activeElement.blur(); }

    showConfirmModal('Are you sure you want to delete this post? This cannot be undone.', function () {
      
      console.log("🔥 [STEP 3] Confirm modal OK clicked! Calling Supabase...");
      ...
      sb.from('forum_posts')
        ...
        .then(function (res) {
          ...
          console.log("🔥 [STEP 4] Supabase responded!", res);

          if (res.error) {
            console.error('🔥 [ERROR] Supabase Error:', res.error);
            showErrorModal('Error deleting post: ' + (res.error.message || 'Unknown error'));
            return;
          }

          if (res.data && res.data.length === 0) {
            console.warn('🔥 [WARNING] 0 rows deleted. The post ID and user ID did not match the database.');
            showErrorModal('Delete failed: You do not have permission or the post does not exist.');
            return; 
          }

          console.log('🔥 [STEP 5] Post deleted successfully!');
        ...
        .catch(function (err) {
          console.error('🔥 [CRITICAL ERROR] Code crashed:', err);
  ...
  function deleteComment(commentId, postId) {
    ...
    showConfirmModal('Are you sure you want to delete this comment? This cannot be undone.', function () {
      sb.from('forum_comments')
        ...
        .then(function (res) {
          if (res.error) {
            console.error('[Forum] Error deleting comment:', res.error);
            showErrorModal('Error deleting comment: ' + (res.error.message || 'Unknown error'));
            return;
          }
          if (res.data && res.data.length === 0) {
            showErrorModal('Delete failed: Item not found or you lack permission.');
            return;
          }
  ...
  function attachConfirmModalListeners() {
    ...
    if (ok) ok.addEventListener('click', function (e) {
      ...
      // Save the callback before hiding the modal so it isn't erased.
  ...
  /**
   * ============================================================================
   * REUSIBLE MENU LISTENER FUNCTION
   * ============================================================================
   */

  /**
   * Reusable function to attach menu listeners to kebab buttons and menus
   * Handles toggle, close on outside click, escape key, and item click actions
   */
  function attachMenuListeners(container) {
    if (!container) return;

    // Kebab button click handlers
    container.querySelectorAll('.kebab-btn').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        if (isOpen) {
          container.querySelectorAll('.kebab-menu:not([hidden])').forEach(function (m) {
            if (m !== menu) {
              m.hidden = true;
              var otherBtn = m.previousElementSibling;
              if (otherBtn && otherBtn.classList.contains('kebab-btn')) {
    ...
    var outsideClickHandler = function (e) {
      ...
      if (!clickedKebab && !clickedMenu) {
        container.querySelectorAll('.kebab-menu:not([hidden])').forEach(function (menu) {
          menu.hidden = true;
    ...
    var escapeHandler = function (e) {
      if (e.key === 'Escape' || e.keyCode === 27) {
        var openMenu = container.querySelector('.kebab-menu:not([hidden])');
        if (openMenu) {
    ...
    // Menu item clicks
    container.querySelectorAll('.kebab-menu-item').forEach(function (item) {
  ...
  /**
   * Render all posts as directory list (forum.html) 
   */
  ...
  function attachEventListenersForDirectory() {
    // Apply reusable menu listener function
    attachMenuListeners(document);
  ...
  function initForumPage() {
    ...
    // mirror same logic as original forum.js so button can't get stuck hidden
    if (user) {
  ...
  function renderPostDetail(post, authorMap) {
    ...
    fetchUserLikes().then(function (likes) {
      ...
      // Apply menu listeners to the detail container
      attachMenuListeners(detail);
    }).catch(function (err) {
  ...
  function renderCommentsForPost(postId, comments, authorMap) {
    ...
    container.innerHTML = html;
    attachEventListenersForPostDetail();
    // Apply menu listeners to the comments container
   ...
   */
  function init() {
    // 1. ADD THIS LOCK: If already initialized for the *same* user, stop immediately.
    var user = getUser();
    ...
    window.isForumInitialized = true; // Mark initialized
    window.lastForumUser = userId;    // remember who we initialized for
